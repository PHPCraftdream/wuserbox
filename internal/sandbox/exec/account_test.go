// The seat the shut tests of this package measure from: a real local sandbox
// account of their own, made the way --init makes one, started with
// RunAsAccount the way a run starts its stub, and removed when the test ends.
//
...
func aShutAccount(t *testing.T, label string) *shutAccount {
	t.Helper()
...
func fixtureTree(t *testing.T, a *shutAccount) (root, work, exe string) {
	...
	// The fixture children write their diagnosis here, and a plain Everyone
	// read of the root does not let them write.
...
func (a *shutAccount) runFixture(t *testing.T, exe, work, flag string) {
	...
	code, err := proc.RunAsAccount(a.name, a.password, line, work, os.Environ())
	if err != nil {
...
// fixtureBroke is the exit code a fixture child answers when its own
// choreography broke: this package's other child processes answer 90 and 91,
// so the fixture's verdict stays unmistakable from theirs.
const fixtureBroke = 95
...
// diagnosis file -- the same split internal/win/proc's fixture works on,
