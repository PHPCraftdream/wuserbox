...
// RulesLock is the name to hold while the standing rules are read, changed
// and written back. The rules file belongs to no single sandbox, so it has a
// name of its own.
const RulesLock = "rules"

// Locked runs work under a name held for the caller alone, waiting for whoever
// holds it now. The name is a sandbox group, or RulesLock for the rules file.
//
...
func take(group string) (func(), error) {
